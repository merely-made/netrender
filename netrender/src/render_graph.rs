...
impl fmt::Display for GraphBuildError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingRequestedOutput { image } => {
                write!(f, "requested output {image:?} is not registered")
            }
            ...
            Self::LegacyTasksPresent => {
                write!(f, "legacy tasks cannot be mixed with planned tasks")
            }
...
impl fmt::Display for RenderGraphError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::PlannedTasksPresent => write!(f, "planned tasks require compile before execute"),
        }
    }
}
...
impl RenderGraph {
    ...
    pub(crate) fn compile(
        ...
        if !self.tasks.is_empty() {
            return Err(GraphBuildError::LegacyTasksPresent);
        }
    ...
    pub fn execute(
        ...
        if !self.planned_tasks.is_empty() {
            return Err(RenderGraphError::PlannedTasksPresent);
        }
